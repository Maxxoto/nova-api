...
class HandleUserPromptUseCase:
    ...
    async def execute(
        self, prompt: str, agent_id: UUID, thread_id: Optional[str] = None
    ...
    async def _store_conversation(
        self, thread_id: Optional[str], prompt: str, agent_state: AgentState
    ) -> None:
        """Store conversation in memory."""
    ...
    async def _update_agent_state(self, agent_state: AgentState, plan: Plan) -> None:
        ...
        await self.memory.update_agent_state(
            agent_state.id, {"capabilities": agent_state.capabilities}
        )

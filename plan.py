...
class Plan(BaseModel):
    ...
    status: str = Field(default="draft", regex="^(draft|active|completed|cancelled)$")
    agent_id: Optional[UUID] = None
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: datetime = Field(default_factory=datetime.utcnow)
    
    def add_step(self, description: str, action: str, parameters: dict = None) -> PlanStep:
    ...
    def update_step_status(self, step_id: UUID, status: str, result: str = None) -> bool:
